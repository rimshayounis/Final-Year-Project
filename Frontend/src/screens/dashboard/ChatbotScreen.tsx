...
export default function ChatbotScreen({ id, role }: ChatbotScreenProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: `Hi ${role === 'doctor' ? 'Doctor' : 'User'}, how can I assist you?`,
      isUser: false,
      timestamp: new Date(),
    },
  ]);
  ...
  const handleOpenCamera = async () => {
    ...
    if (!result.canceled && result.assets?.length > 0) addImageMessage(result.assets[0].uri);
  };
  ...
  const handlePickFile = async () => {
    ...
    if (result.canceled === false) {
      const newMessage: Message = {
        id: Date.now().toString(),
        text: `Sent file: ${result.assets[0].name}`,
        isUser: true,
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, newMessage]);
  ...
  const addImageMessage = (uri: string) => {
    ...
    setTimeout(() => {
      const botResponse: Message = {
        id: (Date.now() + 1).toString(),
        text: 'I see your image! How can I help?',
        isUser: false,
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, botResponse]);
...
const styles = StyleSheet.create({
  ...
  botAvatarContainer: { alignItems: 'center', marginVertical: 20, position: 'relative' },
