...
export default function PatientChatScreen({ route }: Props) {
  const {
    doctorId, doctorName, doctorAvatar, doctorSpecialty, conversationId,
  ...
  const connectSocket = (patId: string) => {
    const socket = io(SOCKET_URL, {
      ...
      query: { userId: patId, role: 'patient' },
    ...
    socket.on('user_stop_typing', ({ userId }: any) => { if (userId === doctorId) setIsDoctorTyping(false); });
    socket.on('user_online',      ({ userId }: any) => { if (userId === doctorId) setIsDoctorOnline(true); });
    socket.on('user_offline',     ({ userId }: any) => { if (userId === doctorId) setIsDoctorOnline(false); });
  ...
  useEffect(() => {
    if (!isDoctorTyping) { [dot1, dot2, dot3].forEach(d => d.setValue(0)); return; }
  ...
  const sendText = useCallback(() => {
    ...
    const temp: Message = {
      _id: `temp_${Date.now()}`, senderId: myId, receiverId: doctorId,
      conversationId, text, createdAt: new Date().toISOString(), read: false, isTemp: true,
    };
    setMessages(prev => [...prev, temp]);
    scrollToBottom();
    socketRef.current?.emit('send_message', { conversationId, senderId: myId, receiverId: doctorId, text });
    socketRef.current?.emit('stop_typing',  { conversationId, userId: myId });
  }, [inputText, doctorId, conversationId, editingMessage]);
  ...
  const handleReact  = (messageId: string, emoji: string) => {
    socketRef.current?.emit('react_message', { messageId, emoji, userId: patientIdRef.current, conversationId });
  };
  ...
  const uploadAndEmit = async (
    ...
    try {
      ...
      const res = await chatAPI.uploadFile(new FormData(), type, {
        uri, name, mimeType, conversationId, receiverId: doctorId,
  ...
  return (
    <View style={styles.container}>
      ...
      <MessageContextMenu
        ...
        isMe={contextMessage?.senderId === patientIdRef.current}
