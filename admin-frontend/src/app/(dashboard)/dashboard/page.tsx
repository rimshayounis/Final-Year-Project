...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [usersRes, doctorsRes, pendingPostsRes, walletRes] = await Promise.allSettled([
        ...
        const allDoctors: any[] = doctors.doctors || [];
        ...
        setStats({
          totalPatients:         (users.users || []).length,
                ...
                <div className="stat-card" key={i}>
                  <div className="stat-top">
                    ...
                    <div className="stat-trend" style={{
                      background: s.action ? '#fee2e2' : '#d1fae5',
                      color:      s.action ? '#dc2626' : '#059669',
                    }}>
          ...
          {stats && !loading && (
            <div className="extra-stats">
              <div className="extra-stat">
                ...
                <div>
                  <div className="extra-val">{stats.totalAppointments}</div>
                  <div className="extra-label">Total Appointments</div>
                </div>
              </div>
              <div className="extra-stat">
                <span className="extra-icon">✅</span>
                <div>
                  <div className="extra-val">{stats.completedAppointments}</div>
                  <div className="extra-label">Completed</div>
                </div>
              </div>
              <div className="extra-stat">
                <span className="extra-icon">📝</span>
                <div>
                  <div className="extra-val">{stats.pendingPosts}</div>
                  <div className="extra-label">Pending Posts</div>
                </div>
              </div>
              <div className="extra-stat">
                <span className="extra-icon">💵</span>
                <div>
                  <div className="extra-val">PKR {stats.totalEarned.toLocaleString()}</div>
                  <div className="extra-label">Total Earned</div>
                </div>
              </div>
            </div>
          )}
            ...
            {loading ? (
              ...
              <>
                  ...
                  <div className="feed-content">
                    <div className="feed-msg">
                      {stats.pendingVerifications > 0
                        ? `${stats.pendingVerifications} doctors pending approval`
                        : 'All doctors verified'}
                    </div>
                    <div className="feed-time">{stats.verifiedDoctors} verified doctors</div>
                  </div>
                  <div className="feed-tag" style={{
                    background: stats.pendingVerifications > 0 ? '#fef3c7' : '#d1fae5',
                    color:      stats.pendingVerifications > 0 ? '#d97706' : '#059669',
                  }}>
                ...
                <div className="feed-item">
                  ...
                  <div className="feed-content">
                    <div className="feed-msg">PKR {stats.totalCommission.toLocaleString()} commission earned</div>
                    <div className="feed-time">PKR {stats.totalEarned.toLocaleString()} total earned</div>
                  </div>
