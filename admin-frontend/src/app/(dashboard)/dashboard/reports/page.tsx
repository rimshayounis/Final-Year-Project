...
export default function ReportsPage() {
    ...
    Promise.allSettled(
      entries.map(({ id, model }) =>
        fetch(`${BASE_URL}/profiles/${model === 'Doctor' ? 'doctor' : 'user'}/${id}`)
          .then(r => r.json()).then(d => ({ id, img: d.data?.profileImage || null }))
          .catch(() => ({ id, img: null }))
      )
    ).then(results => {
  ...
  useEffect(() => {
    ...
    const toFetch = accountReports.filter(r => {
      const rid = typeof r.reportedId === 'string' ? r.reportedId : r.reportedId?._id;
    ...
    Promise.allSettled(
      ...
      results.forEach(r => { if (r.status === 'fulfilled') map[r.value.id] = r.value.banned; });
  ...
  const handleSelectReport = (r: Report) => {
    if (selected?._id === r._id) { setSelected(null); setSelectedPost(null); return; }
    setSelected(r);
  ...
  const deletePost = async (report: Report) => {
    if (!report.postId) { showToast('No post linked to this report', 'error'); return; }
    setUpdating(report._id);
    try {
      const res = await fetch(`${BASE_URL}/posts/admin/${report.postId}`, { method: 'DELETE' });
      if (res.ok) {
        ...
        if (selected?._id === report._id) { setSelected(prev => prev ? { ...prev, status: 'reviewed' } : null); setSelectedPost(null); }
  ...
  const banAccount = async (report: Report) => {
    ...
    if (!reportedId) { showToast('Cannot resolve reported user', 'error'); return; }
    setUpdating(report._id);
    try {
      const res = await fetch(`${BASE_URL}/reports/${report._id}/ban`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ reportedId, reportedModel: report.reportedModel }),
      });
      ...
      if (res.ok) {
        ...
        const msg = data?.message || data?.error || `Failed to ban account (${res.status})`;
        showToast(typeof msg === 'string' ? msg : JSON.stringify(msg), 'error');
      }
    } catch (e: any) { showToast(e?.message || 'Failed to ban account', 'error'); }
    finally { setUpdating(null); }
  };
  ...
  const unbanAccount = async (report: Report) => {
    ...
    } catch { showToast('Failed to unban account', 'error'); }
    finally { setUpdating(null); }
  };
  ...
  const filtered = reports.filter(r => {
    ...
    const matchCategory = category === 'all' || (category === 'posts' ? isPostReport(r) : isAccountReport(r));
  ...
  const renderAvatar = (ref: any, model: 'User' | 'Doctor', size: 'sm' | 'md') => {
    ...
    const bg  = model === 'Doctor' ? { background: '#EEF1FF', color: '#6B7FED' } : { background: '#fee2e2', color: '#dc2626' };
    return img
      ? <img src={`http://localhost:3000${img}`} className={imgCls} alt="" style={{ width: dim, height: dim }} />
      : <div className={cls} style={bg}>{getName(ref)[0]?.toUpperCase() || '?'}</div>;
  };
  ...
  return (
    <div className="rp-page">
      ...
      <div className="rp-category-tabs">
        {([
          { key: 'all',      label: 'All Reports',     icon: '📋', count: reports.length },
          { key: 'posts',    label: 'Post Reports',    icon: '📄', count: postCount },
          { key: 'accounts', label: 'Account Reports', icon: '👤', count: accountCount },
        ] as { key: CategoryFilter; label: string; icon: string; count: number }[]).map(t => (
          <button
            key={t.key}
            className={`rp-cat-tab ${category === t.key ? 'active' : ''}`}
            onClick={() => { setCategory(t.key); setSelected(null); setSelectedPost(null); }}
          >
          ...
          {(['all', 'pending', 'reviewed'] as FilterStatus[]).map(f => (
            <button key={f} className={`rp-filter-btn ${filter === f ? 'active' : ''}`} onClick={() => setFilter(f)}>
              ...
              <span className="rp-filter-count">
                {f === 'all' ? filtered.length : filtered.filter(r => r.status === f).length}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className={`rp-content ${selected ? 'with-detail' : ''}`}>
        ...
        <div className="rp-table-wrap">
          {loading ? (
            <div className="rp-loading">{[1,2,3,4,5].map(i => <div key={i} className="rp-row-skeleton" />)}</div>
          ) : filtered.length === 0 ? (
            <div className="rp-empty">
              <span>{category === 'posts' ? '📄' : category === 'accounts' ? '👤' : '📭'}</span>
              <p>{search || filter !== 'all' ? 'No reports match your filters' : `No ${category === 'all' ? '' : category + ' '}reports yet`}</p>
            </div>
          ) : (
              ...
              <tbody>
                {filtered.map(r => {
                  ...
                  return (
                    ...
                    >
                      <td>
                        <span className={`rp-type-badge ${isPostReport(r) ? 'post' : 'account'}`}>
                      ...
                      <td>
                        <div className="rp-user-cell">
                          ...
                          <div>
                            ...
                            {isAccountReport(r) && (
                              <div className={`rp-ban-pill ${banned ? 'banned' : 'active'}`}>
                      ...
                      <td onClick={e => e.stopPropagation()}>
                        <div className="rp-action-row">
                          {isPostReport(r) ? (
                            <button className="rp-btn-delete" disabled={updating === r._id} onClick={() => deletePost(r)}>
                              🗑 Delete
                            </button>
                          ) : banned ? (
                            <button className="rp-btn-unban" disabled={updating === r._id} onClick={() => unbanAccount(r)}>
                              ✓ Unban
                            </button>
                          ) : (
                            <button className="rp-btn-ban" disabled={updating === r._id} onClick={() => banAccount(r)}>
                              🚫 Ban
                            </button>
                          )}
                          <button className="rp-btn-dismiss" disabled={updating === r._id} onClick={() => dismissReport(r)}>
                            ✕
                          </button>
                        </div>
                      </td>
        ...
        {selected && (
          <div className="rp-detail">
            <div className="rp-detail-header">
              <h3>Report Detail</h3>
              <button className="rp-detail-close" onClick={() => { setSelected(null); setSelectedPost(null); }}>✕</button>
            </div>
            ...
            <div className="rp-detail-section">
              ...
              <div className="rp-detail-user-card" style={{ borderColor: '#6B7FED', background: '#F0F4FF' }}>
                {renderAvatar(selected.reporterId, selected.reporterModel, 'md')}
                <div>
            ...
            <div className="rp-detail-section">
              ...
              <div className="rp-detail-user-card" style={{ borderColor: '#dc2626', background: '#fff8f8' }}>
                {renderAvatar(selected.reportedId, selected.reportedModel, 'md')}
                <div style={{ flex: 1 }}>
                  ...
                  {isAccountReport(selected) && (
                    <div className={`rp-ban-pill ${isBanned(selected) ? 'banned' : 'active'}`} style={{ marginTop: 4 }}>
                      {isBanned(selected) ? '🚫 Currently Banned' : '✓ Currently Active'}
                    </div>
                  )}
                </div>
              </div>
            ...
            {isPostReport(selected) && (
              <div className="rp-detail-section">
                ...
                {postLoading ? (
                  ...
                  <div className="rp-post-preview" style={selectedPost.backgroundColor ? { background: selectedPost.backgroundColor } : {}}>
                    {selectedPost.category && (
                      <span className="rp-post-category">{selectedPost.category}</span>
                    )}
                    {selectedPost.title && (
                      <div className="rp-post-title">{selectedPost.title}</div>
                    )}
                    <div className="rp-post-desc">{selectedPost.description}</div>
                    {selectedPost.mediaUrls && selectedPost.mediaUrls.length > 0 && (
                      <div className="rp-post-media">
                        {selectedPost.mediaUrls.slice(0, 2).map((url, i) => (
                          <img key={i} src={`http://localhost:3000${url}`} className="rp-post-img" alt="" />
                        ))}
                        {selectedPost.mediaUrls.length > 2 && (
                          <div className="rp-post-more-media">+{selectedPost.mediaUrls.length - 2} more</div>
                        )}
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="rp-post-deleted">Post may have been deleted already</div>
                )}
              </div>
            )}
            ...
            <div className="rp-detail-actions">
              ...
              {isPostReport(selected) ? (
                <button className="rp-detail-btn-delete" disabled={updating === selected._id} onClick={() => deletePost(selected)}>
                  🗑 Delete Reported Post
                </button>
              ) : isBanned(selected) ? (
                <button className="rp-detail-btn-unban" disabled={updating === selected._id} onClick={() => unbanAccount(selected)}>
                  ✓ Unban This Account
                </button>
              ) : (
                <button className="rp-detail-btn-ban" disabled={updating === selected._id} onClick={() => banAccount(selected)}>
                  🚫 Ban This Account
                </button>
              )}

              <button className="rp-detail-btn-reviewed" disabled={updating === selected._id} onClick={() => markReviewed(selected._id)}>
                ✓ Mark as Reviewed
              </button>
              <button className="rp-detail-btn-dismiss" disabled={updating === selected._id} onClick={() => dismissReport(selected)}>
